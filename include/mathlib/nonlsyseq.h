...
template <typename F> class nonlinear_equations;

template <typename R, typename... Args> class nonlinear_equations<R(Args...)> {
 ...
 public:
    ...
    nonlinear_equations(const std::initializer_list<function_t>& list)
        : F_(transpose(fmatrix<R(Args...)>{list})), W_(list) {
 ...
 private:
    template <size_t... I> R residual_adapter(int n, const matrix<R>& x, std::index_sequence<I...>) const {
        return residual(n, x[I][0]...);
    }

    template <size_t... I> linear_equations<R> make_syseq(const matrix<R>& x, std::index_sequence<I...>) const {
        using namespace std;
        auto w = W_(x[I][0]...);
        auto f = -F_(x[I][0]...);
    ...
    fmatrix<R(Args...)>
        F_; // �olumn-matrix for system of non-linear equations, it is assumed that each F_[i][0](args...) = 0.
    jacobian<R(Args...)> W_;
};
