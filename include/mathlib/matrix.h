...
template <typename T> class matrix {
    ...
    template <typename iter_t> class row {
     public:
        row() = delete;
        row(row&& /*r*/) = default;
...
template <typename T> static inline matrix<T> operator*(const matrix<T>& m1, const matrix<T>& m2) noexcept(false) {
    if (m1.cols() == m2.rows()) {
...
template <typename T> static inline matrix<T> operator-(const matrix<T>& m) noexcept(true) {
