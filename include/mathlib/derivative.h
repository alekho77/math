...
template <typename F> class derivative;

// Derivative
template <typename R, typename... Args> class derivative<R(Args...)> {
 ...
 public:
    ...
    template <size_t K> R diff(Args... args) const {
 ...
 private:
    template <size_t K, size_t... I> R dh(const R h, const std::tuple<Args...>& vars, std::index_sequence<I...>) const {
        using namespace std;
        auto vars_left = vars;
        get<K>(vars_left) -= h;
    ...
    template <size_t K> inline std::tuple<R, R> make_dh(const R h, const std::tuple<Args...>& vars) const {
        using namespace std;
...
derivative<R(Args...)> make_deriv(R (*func)(Args...), R eps = numeric_consts<R>::epsilon) {
    return derivative<R(Args...)>(func, eps);
}

template <typename C, typename R, typename... Args>
derivative<R(Args...)> make_deriv(R (C::*func)(Args...), C* that, R eps = numeric_consts<R>::epsilon) {
