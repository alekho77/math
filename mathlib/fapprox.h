...
public:
  ...
  const matrix<R>& get_as_matrix() const { return coefs_; }
...
private:
  ...
  template<size_t... I>
  linear_equations<R> make_syseq(const jacobian<R(Args...)>& W, const fmatrix<R(Args...)>& F, const matrix<R>& x, std::index_sequence<I...>) const {
    using namespace std;
    auto w = W(x[I][0]...);
    auto wt = transpose(w);
    w = wt * w;
  ...
  template <size_t... I>
  std::tuple<Args...> make_coef_tuple(std::index_sequence<I...>) const {
